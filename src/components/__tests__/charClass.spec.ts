...
describe('Класс персонажа корректно обрабатывает установку и получение атрибутов', () => {
	...
	test('Добавление оружия к вооружению персонажа и удаление из него', () => {
		...
		expect(unref(char.weapons.value.length)).toBe(0)

		char.addWeapon(EWeapon.longsword)
		expect(unref(char.weapons.value.length)).toBe(1)
		expect(unref(char.weapons.value[0].name)).toBe('Длинный меч')

		// Добавление двух одинаковых оружий
		char.addWeapon(EWeapon.longsword)
		expect(unref(char.weapons.value.length)).toBe(2)
		expect(unref(char.weapons.value[0].name)).toBe('Длинный меч')
		expect(unref(char.weapons.value[1].name)).toBe('Длинный меч')

		char.addWeapon(EWeapon.handaxe)
		expect(unref(char.weapons.value.length)).toBe(3)

		char.removeWeapon(EWeapon.longsword)
		expect(unref(char.weapons.value.length)).toBe(2)
		expect(unref(char.weapons.value[0].name)).toBe('Длинный меч')
		expect(unref(char.weapons.value[1].name)).toBe('Ручной топор')
	})
})