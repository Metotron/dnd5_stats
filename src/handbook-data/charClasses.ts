...
export const fullCharClassesList: TCharClass[] = [{
	...
	diff: {
		...
		features: [
			...
			'[Бардовское вдохновение.] Бонусным действием можно на 1 час вдохновить (+d[:bard/bardicDie:]) существо, которое может вас слышать, в пределах 60 футов. При провале теста d20 это существо может добавить кость вдохновения. Применений:&nbsp;{mod:Харизмы:min1} до продолжительного отдыха',
	...
	diff: {
		...
		features: [
			...
			'[Второе дыхание.] Бонусным действием можете восстановить d10 + {level:} хитов (Применений:&nbsp;[:fighter/secondWind:]). Короткий отдых восстанавливает 1 применение, продолжительный — все применения',
	...
	diff: {
		...
		features: [
			...
			'[Избранный враг.] У вас всегда подготовлено заклинание {spell:Метка охотника}. Можете наложить его [:ranger/favoredEnemy:] раз не тратя ячейку. Количество применений восстанавливается после продолжительного отдыха',
